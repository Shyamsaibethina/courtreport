...
final class Tokeniser {
    ...
    private let charsBuilder: StringBuilder = .init(1024) // buffers characters to output as one token, if more than one emit per read
    let dataBuffer: StringBuilder = .init(1024) // buffers data looking for </script>

    var tagPending: Token.Tag = .init() // tag we are building up
    let startPending: Token.StartTag = .init()
    let endPending: Token.EndTag = .init()
    let charPending: Token.Char = .init()
    let doctypePending: Token.Doctype = .init() // doctype building up
    let commentPending: Token.Comment = .init() // comment building up
